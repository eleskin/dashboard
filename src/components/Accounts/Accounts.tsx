...
import {addWebsite} from '../../store/slices/websites';
import {getFaviconURL, getValidURL} from '../../utils/functions';
...
const Accounts: JSXElementConstructor<any> = ({websites}: { websites: Array<any> }): JSX.Element => {
	...
	const [isEditable, setIsEditable]: [boolean, Dispatch<SetStateAction<boolean>>] = useState(Boolean(false));
	const [websiteURL, setWebsiteURL]: [string, Dispatch<SetStateAction<string>>] = useState('');
	const [websiteURLError, setWebsiteURLError]: [string, Dispatch<SetStateAction<string>>] = useState('');
	...
	const accountsList: Array<JSX.Element> = websites.map((website: any, index: number): JSX.Element => {
		...
		return (
			<Link to={`/home/${url.host}`} className={`${styles.Accounts__account} ${activeAccountIndex === index ? styles.Accounts__account_active : ''}`} key={index}>
	...
	const handleSendURL: MouseEventHandler = (): void => {
		if (websiteURL.length) {
			try {
				dispatch(addWebsite({websiteURL: getValidURL(websiteURL).toString()})).then(() => {
					setIsEditable(false);
					setWebsiteURL('');
				});
			} catch {
				setWebsiteURLError('Incorrect URL');
			}
		}
	};
		...
		>
			...
			<div className={styles.Accounts__dropdown}>
				{!isEditable ? (
					...
					<div className={styles.Accounts__input}>
						<Form.Input
